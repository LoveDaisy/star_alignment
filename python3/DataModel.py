...
if __name__ == "__main__":
    ...
    #logging.basicConfig(format=logging_format, level=logging_level, filename="logs.txt")
    
    data_model = DataModel()
    #img_tmpl = u"/Volumes/ZJJ-4TB/Photos/17.08.21 Eclipse Trip/6DII/IMG_{:04d}_0.tif"
    img_tmpl = u"DSC028{:2d}.tif"
    for p in [img_tmpl.format(i) for i in (26,23,24,25,27,28,29,30)]:
    ...
    #ImageProcessing.save_tif_image("test00.tif", result_img, data_model.images[0].exif_info)
    ...
    for img in data_model.images[1:]:
        ...
        #ImageProcessing.save_tif_image("test{:02d}.tif".format(serial), result_img, data_model.images[0].exif_info)
