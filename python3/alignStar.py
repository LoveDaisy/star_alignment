...
parser.add_argument('-f', '--focalLength', dest='focal', type=float,required=True,
                    help='Focal length')
...
for p in reorder_images(args.images):
    ...
    data_model.add_image(p,focal_length=f,crop_factor=crop_factor)

if data_model.has_image():
    ...
    data_model.accumulate_final_sky( np.copy(ref_img.original_image).astype("float32") / np.iinfo(
    ...
    for img in data_model.images[1:]:
        ...
        try:
            ...
            data_model.accumulate_final_sky(img_tf)
            serial+=1
            if keepInterim:
            ...
            print("Alignment failed for this picture: {}. Discarded.".format(str(e)))

    data_model.update_final_sky()
    ...
    print (f"Done. Output image is in {outputName}")
else:
